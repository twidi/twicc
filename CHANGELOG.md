...
- **Spawn a session and get its answer in one call** — `twicc create-session --wait-reply` keeps going until the new session answers, and hands the reply back with the result instead of leaving you to poll for it. Useful above all for a throwaway worker whose only way of reporting back is what it says. `--reply-timeout` caps the wait at 5 minutes by default — a timeout leaves the session running and tells you where to pick it up, it is not a failure — and `--no-reply-text` reports that the answer arrived without dumping it on you. The wait watches what the session *said*, so a session still busy with a background task answers you right away instead of when it finally goes quiet.
