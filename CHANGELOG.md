...
- **Claude Opus 5.5** — support for Anthropic's new Opus model, now the latest Opus (Opus 5 remains selectable). It costs less than Opus 5.
- **GPT-6 Sol and Luna** — support for OpenAI's new Sol and Luna models, now the latest Sol and Luna (GPT-5.6 Sol and Luna remain selectable). Terra has no GPT-6 version.
- **Send a message and get the answer in one call** — `twicc send-message --wait-reply` keeps going until the session answers, and hands the reply back with the result. It only ever accepts a message written *after* yours, so a fast session that had already finished answering someone else can no longer be mistaken for a reply to you — the trap that made waiting on a session's state unreliable.
- **Spawn a session and get its answer in one call** — `twicc create-session --wait-reply` keeps going until the new session answers, and hands the reply back with the result instead of leaving you to poll for it. Useful above all for a throwaway worker whose only way of reporting back is what it says. `--wait-timeout` caps the wait at 5 minutes by default — a timeout leaves the session running and tells you where to pick it up, it is not a failure — and `--no-reply-text` reports that the answer arrived without dumping it on you. The wait watches what the session *said*, so a session still busy with a background task answers you right away instead of when it finally goes quiet.
...
- **v1.94.2: Subagents all the way down** — nested subagents and their tree in the Orchestration tab, one-shot ephemeral sessions, and batched MCP commands.

### Added

- **MCP batch commands** — Collect independent reads or run ordered commands in one internal or external MCP call, with per-command results and permission checks.
...
- **v1.94.1: External MCP** — connect an assistant outside TwiCC to your instance, under your approval, plus fixes for background sessions and Codex goals.
