...
- **Paging (`--paginated`).** Every listing takes `--limit` / `--offset`, but a bare page never says whether another one follows. Pass `--paginated` and the result becomes `{"items": [...], "pagination": {"limit", "offset", "total", "has_more"}}` — same four keys on every command, so `has_more` answers the question without a probe request. Without the flag the output shape is exactly what it has always been; nothing to migrate. Accepted by `projects`, `workspaces`, `sessions`, `session content` / `messages` / `agents` / `workflows`, `artifacts`, `share`, `processes` and `search`. Two specifics: with `session messages --tail N` the window is reported as the range it covers and `has_more` means "messages remain *before* it"; on that same command without `--contains`, `total` counts raw items, a handful of which extract to nothing, so `has_more` can be a rare false positive — never a false negative.
...
- Listing: `--project TEXT`, `--workspace TEXT`, `--limit` (default 20), `--offset`, `--paginated`, `--include-archived`, plus the shared filiation/visibility/annotation filters (see below). `--project` and `--workspace` (mutually exclusive) each fold in git worktrees — a worktree's sessions belong to its main repository — mirroring the UI.
...
- `content [RANGE] [--contains TEXT ...]` — raw item content by line number/range (e.g. `5`, `10-20`) and/or substring. `--contains` is case-insensitive and repeatable (AND-combined). Also takes `--limit`, `--offset`, `--paginated`, applied **after** the range and `--contains`: the range is an absolute address in the JSONL, the window is a rank in the matches, and they stack. At least one selector (`RANGE`, `--contains`, `--limit`/`--offset`) is required — a bare call would dump the whole session, the heaviest payload the CLI can produce. No match is an empty result, not an error. Each result is `{line_num, content}`.
- `messages [--contains TEXT ...]` — all user/assistant messages, cross-provider, uniform shape. Options: `--range`, `--role user|assistant`, `--contains TEXT` (case-insensitive substring on the extracted message text, repeatable/AND-combined, applied before paging), `--limit`, `--offset`, `--paginated`, `--tail N` (last N; mutually exclusive with `--limit`/`--offset`).
- `agents` — list subagents. Options: `--limit` (default 20), `--offset`, `--paginated`.
...
- `workflows` — list the session's workflows (Claude Code only). Options: `--limit` (default 20), `--offset`, `--paginated`.
