...
- `wait [--from N] [--reply] [--blocked] [--wait-timeout N] [--no-reply-text]` — block until the session says something past the cursor. The other waits ride on a command that triggered the turn, so their cursor falls out of the send; here nothing was sent, so `--from` names the line to start above — the `line_num` or `since_line_num` a previous wait handed back, which is what makes a timed-out wait resumable. Omitted, the cursor is the session's current last line ("tell me the next thing it says"), and an idle session reports `ended` rather than hanging: unlike `process wait --transition`, a session that has stopped is observable — though not instantly, since the loop waits out a ~5 s flush window first, so a shorter `--wait-timeout` can only ever report `timeout`. An answer always ends the wait — `--reply` names that default rather than switching it — and `--blocked` **adds** a second ending, the session blocking on a human, with an answer still winning a tie: `--blocked` behaving as `--wait-blocked` does on the commands that send (`--reply` has no counterpart: `--wait-reply` turns the wait on, and here the command is the wait). Returns the same `reply` block as `--wait-reply`. **The exit code says how it went**, since waiting is the command rather than a follow-up: `0` answered (or blocked, with `--blocked`), `5` no answer came (`timeout`, `ended`, `provider_error`), `2` TwiCC stopped, `1` a local refusal (bad `--from`, non-positive `--wait-timeout`, unknown session) or the wait itself breaking.
