...
- `wait [--from N] [--since INSTANT] [--reply] [--blocked] [--wait-timeout N] [--no-reply-text]` — block until the session says something past the cursor. The other waits ride on a command that triggered the turn, so their cursor falls out of the send; here nothing was sent, so `--from` names the line to start above — the `line_num` or `since_line_num` a previous wait handed back, which is what makes a timed-out wait resumable. `--since` names that same cursor as an ISO 8601 instant instead — mutually exclusive with `--from`, the wait starting above the last line written at or before that moment, so "past line N" and "after time T" select the same lines; no offset means UTC, the form this CLI stores and prints, and a bare date means its midnight. Use it when you have a time and not a `line_num`, or when one moment must address several sessions — line 42 is a different place in every transcript. Omitted, the cursor is the session's current last line ("tell me the next thing it says"), and an idle session reports `ended` rather than hanging: unlike `process wait --transition`, a session that has stopped is observable — though not instantly, since the loop waits out a ~5 s flush window first, so a shorter `--wait-timeout` can only ever report `timeout`. With no flag, the wait ends on an answer: `--reply` names that default, and `--blocked` **ORs in** a second ending, the session blocking on a human, with an answer still winning a tie — the combination `--wait-reply` / `--wait-blocked` give on the commands that send, the only difference being that there the wait must be turned on and here the command is the wait. Returns the same `reply` block as `--wait-reply`. **The exit code says how it went**, since waiting is the command rather than a follow-up: `0` answered (or blocked, with `--blocked`), `5` no answer came (`timeout`, `ended`, `provider_error`), `2` TwiCC stopped, `1` a local refusal (bad `--from`, non-positive `--wait-timeout`, unknown session) or the wait itself breaking.
...
- **Wait for the answer:** `--wait-reply` keeps going after the session is created, until it answers, and adds a `reply` block to the result: `outcome`, `line_num`, `is_final`, `since_line_num`, `waited_seconds`, the answer's `text`, and an `error` string on `wait_failed` (drop it with `--no-reply-text` when you only need the go-ahead, not the payload in your context). `outcome` is `replied` (the message closing the turn), `provider_error` (the provider refused the turn: quota, outage — your request was not the problem), `ended` (the turn is over and nothing closed it: a crash, an interruption, or an empty answer), `timeout`, `backend_gone` (TwiCC stopped or restarted, so nothing could be observed), or `wait_failed` (the wait itself broke; the session is unaffected). An agent blocked on a click in the UI does **not** end the wait — a human can still answer — so the result instead carries `"awaiting_user_input": true` alongside whatever ended it. The wait reads the transcript, not the process state, so a session held busy by a Monitor or a live subagent reports as soon as it speaks instead of hours later. `--wait-timeout` caps it (default **300 s**, the ceiling MCP callers are asked to respect — an MCP client may itself give up on a tool silent that long, so over MCP pass a shorter value and come back; no way to wait forever). `--wait-blocked` adds a second way for the wait to end: the session blocking on a human, reported as `outcome: awaiting_user_input`. The two are OR-combined and an answer wins a tie; it is off by default because a block only strands a wait when nobody is there to clear it, which is the `--hidden` case where blocking cannot happen. **A timeout is not a failure** — the agent keeps working and `since_line_num` says where to resume; the exit code never reflects the wait, only whether the session was created.
- Plus `--timeout`.
...
- **Wait for the answers:** `--wait-reply` adds the same `reply` block `send-message` returns, per entry, plus `replied` / `all_replied` to the summary — each recipient waited from its own cursor, only those that reached `sent`. `--wait-first` stops at the first answer and leaves the rest `outcome: pending`; `--wait-all` (default) waits for every one. A recipient whose turn crashed never ends a `--wait-first` batch. `--wait-blocked` and `--no-reply-text` behave as on `send-message`; `--wait-timeout` (default 300 s) is one wall-clock budget for the whole batch, since they are waited on together.
