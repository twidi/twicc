...
- **Paging (`--paginated`).** Every listing takes `--limit` / `--offset`, but a bare page never says whether another one follows. Pass `--paginated` and the result becomes `{"items": [...], "pagination": {"limit", "offset", "total", "has_more"}}` — same four keys on every command, so `has_more` answers the question without a probe request. Without an explicit `--limit` the page size is **20**, the page size every listing uses from 2026-10-01: the flag promises a page, so it supplies one (`--tail` keeps its own window). **Before 2026-10-01 the flag is opt-in; from that date the envelope is the only shape and `--paginated` is an accepted no-op** — the switch happens at local midnight on the machine running TwiCC. While the flag is opt-in, a call without it returns exactly what it always did — except `share`, whose default page is 20 now (it was 50), and the session commands' rows, which gain new keys and values now (see *Slim listings*) — plus a one-line notice on stderr (and in the RPC envelope's `warnings` key) saying what is coming; agents on MCP are never notified, since they read the flag off the tool schema and adapt on their own. Either way a script that passes `--paginated` keeps working untouched. Accepted by `projects`, `workspaces`, `sessions`, `session content` / `messages` / `agents` / `workflows`, `artifacts`, `share` and `search`, and, as `{items}` only (no `pagination`: one entry per id asked, or every approved peer), by `sessions get`, `projects get`, `workspaces get` and `peers`. Two specifics: with `session messages --tail N` the window is reported as the range it covers and `has_more` means "messages remain *before* it"; on that same command, when nothing filters after extraction (no `--contains`, and no `--is-final` or one listing all three values), `total` counts raw items, a handful of which extract to nothing, so `has_more` can be a rare false positive — never a false negative. Migration details: [CLI / RPC migration of October 1, 2026](frontend/public/help/cli-rpc-migration-2026-10-01.md).
- **Slim listings (`--slim` / `--full`).** A command that returns sessions returns a reduced projection of each — every field except the payloads you fetch per session (`tasks`, `plan_paths`, `goals`, `layout`; the `has_*` flags say what is there), the redundant timestamps (`mtime`, `last_started_at`, `last_updated_at`, `last_stopped_at`, `last_viewed_at`), the cost breakdown (`self_cost`, `subagents_cost`), `slug`, `browser_url` and `compute_version_up_to_date` — with a `process` block reduced to `{state}`. Every session row these commands emit (`sessions`, `sessions get`, `session agents`, `session <ID>`, `whoami`'s new shape — not `topology` nodes) also carries `project_directory`, `scratch_dir`, `orchestration_scratch_dir` (`null` outside an orchestration) and `question_widget`; `artifacts_dir` is always the session's folder path (over MCP / RPC `has_artifacts` says whether it holds anything; from a terminal `has_artifacts` is always `false`, so check the folder itself), and the agent settings are effective — the stored value, else the current default — except on a subagent row, which keeps its stored values. `--full` returns the full payload instead, whatever the date; `--slim` and `--full` are mutually exclusive (exit `2`). **Before 2026-10-01 the full payload is still the default and `--slim` opts in; from that date the reduced projection is the default and `--slim` is an accepted no-op** — same local-midnight switch as `--paginated`. Until then, a call with neither flag returns what it always did — plus the keys added now (`project_directory`, `scratch_dir`, `orchestration_scratch_dir`, `question_widget`), with `artifacts_dir` always a path and the agent settings effective — and a one-line notice on stderr (and in the RPC envelope's `warnings` key), never on MCP; a flagless `sessions`, `sessions get` or `session agents` call therefore prints two notices, the pagination (or lookup) one and this one; `session <ID>` and `whoami` print one (on the terminal only for `whoami`). Accepted by `sessions`, `sessions get`, `session agents`, `session <ID>`, `whoami` and `topology`, whose sessions are already reduced and whose `process` block is the part that changes (no notice with `--no-processes`). `session <ID>` and `whoami` follow the same flags and date (`whoami`'s flagless call keeps its own object until then). Migration details: [CLI / RPC migration of October 1, 2026](frontend/public/help/cli-rpc-migration-2026-10-01.md).
...
Then run `$TWICC <args>` — **never quote `$TWICC`** (it may expand to multiple words). `twicc info` also returns the canonical invocation under `twicc_executable`.
...
`twicc whoami` is the explicit way for an agent to discover its own id (`session_id` in the flagless output until 2026-10-01; `id` with `--slim` / `--full`, and from that date), working directories, settings, and live process row.
...
### `twicc whoami`
Print details of the session that owns the calling process. With `--slim` / `--full` (usable now), and from 2026-10-01 without a flag: the `session self` payload — the session row, reduced or in full, with its `process` block inside (`id`, `git_directory`, the agent settings as top-level fields). Until 2026-10-01 a flagless call keeps its current object — `session_id`, `title`, `project_id`, `project_directory`, `current_working_directory`, `artifacts_dir`, `scratch_dir`, `orchestration_scratch_dir` (when part of an orchestration), the resolved `agent_settings`, the `session` sub-object, and the nine-field `process` row — plus a notice on stderr in a terminal. Exits `1` from a plain terminal (only meaningful inside a session), `2` for `--slim --full`.
...
- `--effort TEXT` — default reasoning effort (`low`, `medium`, `high`, `xhigh`; `max` also on Claude Code and on GPT-6 and GPT-5.6 Codex models; aliases `min`/`max`).
...
- `get`: takes one or more project ids or directory paths (no filter flags) and `--paginated`; each input yields one entry in input order, archived included, with `known: false` placeholders for misses. A bare array until 2026-10-01 (with a notice), `{"items": [...]}` (no `pagination`) from that date or with `--paginated` now.
...
- `get`: one or more ids (no filter flags) and `--paginated`, input order preserved, archived included, `known: false` for misses. A bare array until 2026-10-01 (with a notice), `{"items": [...]}` (no `pagination`) from that date or with `--paginated` now.
...
- Listing: `--project TEXT`, `--workspace TEXT`, `--limit` (default 20), `--offset`, `--paginated`, `--slim` / `--full`, `--include-archived`, plus the shared filiation/visibility/annotation filters (see below). `--project` and `--workspace` (mutually exclusive) each fold in git worktrees — a worktree's sessions belong to its main repository — mirroring the UI.
- `get`: one or more ids (no filter flags beyond `--slim` / `--full` and `--paginated`), input order preserved; subagents, archived and hidden sessions all returned, `known: false` for misses. Same projection as the listing, on the placeholders too. A bare array until 2026-10-01 (with a notice), `{"items": [...]}` (no `pagination`) from that date or with `--paginated` now.
- **Live process state:** every row carries a `process` block. `{state}` in the reduced projection, plus `id`, `started_at`, `last_state_change_at` and `pid` with the full payload. `state` is one of `starting`, `assistant_turn`, `awaiting_user_input`, `user_turn`, `dead` — note `awaiting_user_input`, the non-obvious stop: the agent is blocked on a human, not working. It answers **what TwiCC is running**, so `dead` means no TwiCC-managed process, which is also the answer for the many sessions TwiCC indexed but never started (a `claude` run straight from a terminal keeps generating and still reads `dead`). `dead` also covers a TwiCC that is not running at all: an agent does not outlive its backend, so "TwiCC is down" and "TwiCC runs nothing for this session" are the same fact. The block is `null` for one case only — a subagent, which runs inside its parent's process and never has one of its own; `parent_session_id` identifies those.
- **Filtering on it:** `--state <VALUE>` keeps one bucket — **repeatable and OR-combined**, since a session holds a single state, so `--state assistant_turn --state awaiting_user_input` means "busy or blocked" and naming all five is the unfiltered listing. `dead` is accepted and means "no TwiCC-managed process"; `--active` keeps every state but `dead` — `user_turn` included, the agent is loaded and idle, not gone. The two are mutually exclusive. `--provider claude_code|codex` is an ordinary column filter, unrelated to state — an unknown value is refused (exit 1) rather than silently matching nothing. The state filter runs **before** the page is cut, so `total` and `has_more` count what matches. Hidden sessions stay hidden unless you ask (`--include-hidden`, or any filiation scope, which lifts it for you).
- **`sessions wait-reply [SESSION_ID...] [--since INSTANT] [--wait-first|--wait-all] [--wait-timeout N] [--no-reply-text]`** — block until several sessions conclude, past their own cursors. The plural of `session <ID> wait-reply`, for sessions nobody just messaged — or ones you messaged with `send-messages` without `--wait-reply` (then pass `--since` an instant taken before the send, or wait on each with `session <id> wait-reply --from <last_line>`); when you send the messages, the simplest form is `send-messages --wait-reply`, which reads the cursors for you. One loop, **one wall-clock budget** for the batch. Each ends on an answer or a **pending request** only a human can clear, an answer winning a tie; `--wait-all` (default) waits for every one, `--wait-first` stops at the first to conclude and leaves the rest `outcome: pending`. **A bare call is refused** — at least one id or one filter, since the listing's empty case would mean polling every indexed session. Filters: `--project`, `--workspace`, `--provider`, `--state` (minus `dead`, refused: no process means it will never speak), `--active`, `--only-hidden`, `--spawned-by`, `--spawn-tree`, `--descendants`, `--siblings` and `--annotation`, with ids unioned on top. `--include-hidden` and `--include-archived` do not apply — hidden always on, archived always off — while `--only-hidden` still narrows. Each session starts after the session's last user message, so an answer already given is returned (except while a session's compute is not current — e.g. right after a TwiCC restart: then pass `--since` an instant before the spawn or the send), or above the instant `--since` names (an ISO 8601 instant; no offset means UTC; a bare date means its midnight UTC), translated per session — there is no `--from`, line 42 being a different place in every transcript. Re-running resumes, except for a session whose compute is not current; `--since <the instant the batch started>` resumes in every case. Returns `summary` + `results`, one `reply` block per id; `summary` carries `total` (every id asked for, unknown ones included), `replied`, `awaiting_user_input`, `concluded` and `all_replied`, where `replied` counts `outcome: replied` alone while `concluded` also counts pending requests, a named id with a live process but no indexed transcript yet (just spawned) is waited on from line 0, `--since` not applying to it, and a named id with neither a session nor a live process comes back as `outcome: unknown_session`, carrying only that and `session_id`. The filters select indexed sessions only, so a child spawned seconds ago is reached by naming its id, and a barrier on a subset names only that subset's ids. Exit `0` whatever the outcomes — branch on `summary.all_replied` or on each `outcome`; `1` on a local refusal. **Known limit (Claude Code), rare and accepted:** a message sent while a Claude session is busy is queued, not recorded as a user message, so every wait for an answer — `--wait-reply` included, with a cursor or the default — returns the first final message past its cursor, the running turn's closing message; usually that turn read the queued message and covers it, but when the queued message runs as a turn of its own afterwards, the wait returns an answer that does not cover it.
- **`sessions stop [SESSION_ID...]`** — stop the agents behind selected sessions. Only sessions that currently have a process are targeted. **A bare call is refused** (exit `1`): at least one id or one filter; "stop everything running" is written on purpose with `--state starting --state assistant_turn --state awaiting_user_input --state user_turn`. Takes the same selection as the listing (`--project`, `--workspace`, `--provider`, `--state`, the filiation scopes, `--annotation`) plus `--timeout` (default 30, a wall-clock budget for the whole batch) and `--force` (SIGKILL, no grace window). Named ids are added to the filters' selection (unioned), so `sessions stop <MANAGER_ID> --descendants <MANAGER_ID>` stops a manager and its subtree. **The caller is never stopped:** named (`self` included) or selected by a filter, it is reported `skipped_self` (counted in `failed`, nothing sent; `session self stop` stops it). `parent`, `--spawn-tree` and `--siblings` still reach beyond the caller's children (`--spawn-tree self` selects it, reported `skipped_self`); `--annotation` alone still selects across every tree, so pair it with a filiation scope. Filter-selected sessions are narrowed to live ones: one entry per running target. **Hidden sessions are included** — they are the orchestration convention, and sparing them would leave the workers running; archived ones never are, since archiving already kills the agent. `--state dead` is refused: there is nothing to stop. Idempotent. Returns `{summary, results}`: `summary` holds `total`, `succeeded` (`status: stopped`), `failed` and `all_succeeded` (`true` on an empty selection); `results` maps each target id, in selection order, to its entry (`session_id`, `session_known`, `status` — `stopped` / `rejected` / `failed` / `timeout` / `skipped_*`, `skipped_self` included —, `request_uuid`, `provider`, `session_title`, `project_id`, `error`). Exit 0 whatever the per-id outcome; exit `1` on a local refusal before anything is stopped: a bare call, `--state dead`, `--timeout` ≤ 0 or conflicting scopes are refused even with no backend; an unknown `--workspace` / `--provider` / `--state` value, a malformed `--annotation` or a `self` / `parent` that cannot be resolved are checked after the backend check, so with no backend they exit `2`; exit `2` when no backend runs.
...
- Default (no sub-command) — `session <SESSION_ID|self|parent> [--slim|--full]`: the session row, `process` block included — the same one `sessions` joins onto every row, `null` on a subagent. Full until 2026-10-01 (with a notice on a flagless call), reduced from that date; `--slim` / `--full` choose, before or after the id, never before a subcommand (`session <ID> --full agents` exits 2). `self` / `parent` resolve to your own session / its spawner. Any session row is served, one with no user message yet included. The same row `sessions get <SESSION_ID>` returns for one id, minus its `known` flag — reach for that one for several ids. Exits 1 when no session row has that id, or when `self` / `parent` cannot be resolved.
...
- `stop` — stop this session's live agent (`--timeout`, `--force`).
- `wait-reply [--from N] [--since INSTANT] [--wait-timeout N] [--no-reply-text]` — block until the session concludes past the cursor. The other waits ride on a command that triggered the turn, so their cursor falls out of the send; here nothing was sent by this command, so `--from` names the line to start above — the `line_num` or `since_line_num` a previous wait handed back, which is what makes a timed-out wait resumable. Use it on a session nobody just messaged — or one you messaged yourself without `--wait-reply` (then pass `--from` the `last_line` the send returned). `--since` names that same cursor as an ISO 8601 instant instead — mutually exclusive with `--from`, the wait starting just below the first line stamped strictly after that moment. A timestamp that goes backwards can only push the cursor lower, never past a line, and a line with no timestamp is not a boundary, one below the boundary not being re-scanned; no offset means UTC, the form this CLI stores and prints, and a bare date means its midnight. Use it when you have a time and not a `line_num`, or when one moment must address several sessions — line 42 is a different place in every transcript. Omitted, the cursor is after the session's last user message, so an answer already given is returned at once (except while a session's compute is not current — e.g. right after a TwiCC restart: then pass `--since` an instant before the spawn or the send); an idle session with **no** answer past the cursor reports `ended` rather than hanging: a session that has stopped is observable — though not instantly, since the loop waits out a ~5 s flush window first, so a shorter `--wait-timeout` can only ever report `timeout`. `--from` / `--since` exist here and on `sessions wait-reply` (`--since` only), not on `create-session`, `send-message` or `send-messages`. Two endings, and no flag chooses between them: an answer, or a **pending request** — a tool approval or a question, which only a human can clear, so riding the budget out would buy nothing. An answer arriving in the same poll wins. That is exactly what `--wait-reply` does on the commands that send, which is why it carries the same name, and it returns the same `reply` block. **The exit code says how it went**, since waiting is the command rather than a follow-up: `0` answered or blocked, `5` neither came (`timeout`, `ended`, `provider_error`), `2` TwiCC stopped, `1` a local refusal (a bad `--from` or `--since`, the two cursors passed together, a non-positive `--wait-timeout`, an id with no session and no live process) or the wait itself breaking. A session with a live process but no indexed transcript yet (just spawned) is waited on from line 0, or from `--from`; `--since` does not apply to it. `session self wait-reply` is refused (exit `1`): the caller is mid-turn, so its own answer could never come before the deadline.
- `pending-requests [--raw] [--timeout N]` — what this session's live agent is waiting on. Lists **every** pending request, not only the ones answerable here: a session frozen on a tool approval is waiting even though nothing in the CLI can unblock it, and an empty list would say the opposite. Each entry is `{request_id, created_at, kind, ...}`. `kind: "question"` carries `age_seconds`, the `questions` (`id`, `header`, `question`, `multi_select`, `allows_free_text`, `secret`, `options`) and the `actions` you may take; `kind: "out_of_scope"` carries a `reason` (`tool_approval`, `mcp_tool_approval`, `elicitation`, `terminal_only`, `choice`) and an empty `actions`. `--raw` adds each request's untouched `tool_input` — on every entry, so asking for it on a session blocked on a large patch returns that patch. **Emptiness is not an error:** nothing pending and no agent at all both exit 0, the second with `agent_state: "dead"`.
- `answer-questions [--request-id ID] [--choice 'ID=VALUE'] [--timeout N]` and `cancel-questions [--request-id ID] [--timeout N]` — answer, or decline, the question the agent is waiting on. Only a question: a tool approval or an MCP elicitation is refused (`not_a_question`) and stays the UI's business. `--choice` splits on the **first** `=`, so a value may contain more; repeat it once per question, or several times on one id for a multi-select question. A value matching no option is free text, accepted when the question's `allows_free_text` says so, and exclusive of the options. **The id comes from `pending-requests`** and is not the same thing on both providers: a 1-based index on Claude Code, which has no question ids, and the native id on Codex. `--request-id` is optional when one question is pending and required when several are; a script that read `pending-requests` should pass it anyway, so a request that cleared in between surfaces as `request_gone` instead of a wrong answer. Answering every question submits, answering some sends the agent a partial answer where the provider has one (Claude Code only), answering none is refused — declining is what `cancel-questions` is for. **A session cannot answer its own question.** Rejections (exit 3): `no_pending_question`, `ambiguous_request`, `not_a_question`, `request_gone`, `agent_dead`, `missing_answers`, `unknown_question_id`, `free_text_not_allowed`, `free_text_exclusive`, `multi_select_unsupported`, `secret_answer_unsupported`, `option_not_accepted`, `self_answer_refused`, `provider_disabled`.
- `agents` — list subagents. Options: `--limit` (default 20), `--offset`, `--paginated`, `--slim` / `--full` (reduced projection by default from 2026-10-01, see "Slim listings"). Rows carry a `process` block too, always `null`: a subagent runs inside its parent's process.
- `plan [PATH] [--list]` — the session's tracked plan documents (`Session.plan_paths` — native Claude plan + detected plans/specs/handoffs..., incl. subagent-written, both providers). Default: the content of the most recently updated one, as `{path, abs_path, content}`; errors when the session tracks none. With `PATH`: that document, matched against the tracked entries only (stored project-relative/absolute path as shown by `--list`, or its resolved absolute form — never an arbitrary filesystem path). `--list`: every tracked entry, newest first, enriched with a resolved `abs_path` (worktree-aware) and fresh `exists` — same entries as `plan_paths` in `session <ID> --full` (minus `abs_path`).
...
List (read-only, direct DB — works with the server down) or show one share as JSON. Listing: `--kind <session|artifact>`, `--session <ID|self|parent>`, `--project TEXT` (worktree-aware scope), `--include-revoked`, `--limit` (default 20; it was 50), `--offset`, `--paginated`. Each row is the owner serializer (`id`, `token`, `kind`, `label`, `status`, `options`, `view_count`, …) plus a resolved `url` (absolute when `shareBaseUrl` is set, else the relative `/share/<token>/` path).
...
### `twicc peers`
List peer instances as `{id, name, state, broken_reason, last_contact_at}` — `active` (messageable) and `broken` (revoked or unreachable, kept so a failing send can be explained). The list sits under `peers` (`{"peers": [...]}`) until 2026-10-01, with a notice; from that date it sits under `items` (`{"items": [...]}`, no `pagination`). Takes `--paginated`, which opts into `items` now.
...
**Deprecated until 2026-10-01; from that date it refuses to run** (exit `64`, an error naming the replacement). Until then it works as before (`--paginated` included), with a notice naming the replacement. Migration details: [CLI / RPC migration of October 1, 2026](frontend/public/help/cli-rpc-migration-2026-10-01.md). Replacements:
- `processes [filters]` → `sessions --active [filters]` (or `--state …`). A session not indexed yet is not listed (read it with `sessions get <id>`); from 2026-10-01 the row's `process` block is `{state}` unless `--full`.
- `processes get <ids>` → `sessions get <ids>` (`--full` for `pid` and timestamps). `known` means "a `Session` row exists", and a live `process` block can ride on `known: false`; the process fields sit under `process.*`, not flat. Returns `{items}` from 2026-10-01 (`--paginated` now).
- `processes stop` → `sessions stop`, with wider selection; it refuses a bare call, never stops the caller (`skipped_self`) and returns `{summary, results}` (see its section).
- `processes wait` → `sessions wait-reply`: it waits for an answer, not a state, and exits `0` whatever the outcomes. Waits on a lifecycle state and `--transition` have no replacement.
- Skill: [`twicc-processes`](src/twicc/agent/plugin/twicc/skills/twicc-processes/SKILL.md).

### `twicc process <SESSION_ID> <SUBCOMMAND>`
**Deprecated until 2026-10-01; from that date it refuses to run** (exit `64`, an error naming the replacement). Migration details: [CLI / RPC migration of October 1, 2026](frontend/public/help/cli-rpc-migration-2026-10-01.md). Replacements:
- `process <id>` → `session <id>` (or `sessions get <id>` right after a spawn): exit `0` with `process.state: "dead"` when nothing runs, instead of exit `1`. From 2026-10-01 the row is reduced and its `process` block is `{state}`; `--full` for `pid` and timestamps.
- `process <id> stop` → `session <id> stop`, same options.
- `process <id> wait` → `session <id> wait-reply`: it waits for an answer, not a state (exit `0` replied or blocked, `5` `timeout` / `ended` / `provider_error`, `2` `backend_gone`, `1` refusal or `wait_failed`). Waits on a lifecycle state and `--transition` have no replacement.
...
- `--slim` / `--full` — `--full` emits the full `session` serialization per node instead of the reduced subset, and the five-field `process` block. By default each node's `process` block keeps its five fields until 2026-10-01 and is `{state}` alone from that date; `--slim` opts in now and is a no-op after. `--full-sessions` is a deprecated alias of `--full`.
...
`sessions` and `search` accept these cross-cutting filters (`topology` takes the annotation/siblings variants as node markers, see below):
...
Batch mutation is narrower on purpose: `update-sessions` accepts `--spawned-by <ID|self>` or `--descendants <ID|self>` plus optional `--annotation`, but not `parent`, not `--spawn-tree`, and not `--siblings` — you observe and message peers, you don't mutate them. `sessions stop` and `sessions wait-reply` take the listing's whole selection; use them scoped (the `sessions stop` guardrails are in its entry above). `send-messages` is the one batch command that does take `--siblings` (peer broadcast).
