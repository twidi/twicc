...
- Default (no sub-command) — the full session row, `process` block included: the same one `sessions` joins onto every row, `null` on a subagent. Identical to `sessions get <SESSION_ID>` for a single id, which is the one to reach for when you want several at once or `--slim`.
