...
- `agents` — list subagents. Options: `--limit` (default 20), `--offset`, `--paginated`, `--slim`, `--processes/--no-processes` (every row is a subagent, so the block is always `null` — it is emitted anyway so the three listings keep one shape).
