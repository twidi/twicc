...
- `get`: one or more ids (no filter flags beyond `--slim` / `--full`), input order preserved; subagents, archived and hidden sessions all returned, `known: false` for misses. Same projection as the listing, on the placeholders too.
...
- `processes get <ids>` → `sessions get <ids>` (`--full` for `pid` and timestamps). `known` means "a `Session` row exists", and a live `process` block can ride on `known: false`; the process fields sit under `process.*`, not flat.
- `processes stop` → `sessions stop`, with wider selection and no guardrail (see its section).
