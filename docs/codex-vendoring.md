...
**None right now.** The `SubAgentActivityKind` patch carried from 2026-08-27 was dropped at the `rust-v0.155.1` re-vendor: upstream now declares `completed` itself. The vendored tree is identical to the pristine one, so the `diff -rq` above must come back empty.
