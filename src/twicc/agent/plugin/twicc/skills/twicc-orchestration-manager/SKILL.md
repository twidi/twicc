...
Repeat the same call, naming only the ids whose `results[id].outcome` is `timeout`. Re-running resumes, except for a session whose compute is not current; `--since <the instant the batch started>` resumes in every case. The other outcomes and the later-round cursor are in `twicc-orchestration` (*Wait only on your direct children*).

## Handle failures locally first

If a child fails, deal with it yourself first — retry, re-split, or spawn a replacement. If you intentionally abort selected children, stop them by id (`$TWICC sessions stop <ID>... --timeout 30`) or with a scoped batch such as `$TWICC sessions stop --spawned-by self --annotation status=cancelled --timeout 30` — a bare call is refused, and `sessions stop` never stops you (`skipped_self`). Escalate to your parent (`send-message parent`) only when the mandate is genuinely blocked.
