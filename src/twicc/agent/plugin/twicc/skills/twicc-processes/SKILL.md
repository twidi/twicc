...
| `processes get <ids>` | `sessions get <ids>` (`--full` for `pid` and timestamps) | `known` means "a session row exists"; a live `process` block can ride on `known: false`. The process fields sit under `process.*`, not flat on the entry. Returns `{items}` from 2026-10-01 (`--paginated` now). |
| `processes stop [ids] [filters]` | `sessions stop [ids] [filters]` | A bare call is refused (at least one id or one filter), and it never stops the caller: the calling session is reported `skipped_self` (use `session self stop`). Returns `{summary, results}` instead of an array. `parent`, `--spawn-tree` and `--siblings` still reach beyond your children. `--annotation` alone selects across every tree: pair it with a filiation scope. Named ids are unioned with the filters. |
| `processes wait` | `sessions wait-reply <ids>` | Waits for an answer, not a state. Exit `0` whatever the outcomes: branch on `summary.all_replied` and each `results[id].outcome`. `--wait-first` replaces `--first` and also stops on `awaiting_user_input`. `--wait-timeout` (≤ 300 s) replaces `--timeout`; a longer wait repeats the call — re-running resumes, except for a session whose compute is not current; `--since <the instant the batch started>` resumes in every case — naming only the ids still on `timeout` — `ended`, `provider_error` and `unknown_session` are final (skill: `twicc-orchestration`). |
