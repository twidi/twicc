...
- `stop` — stop this session's live agent (`--timeout`, `--force` for a SIGKILL without the grace window). Idempotent: stopping an already-stopped session still reports `stopped`.
...
| `role: user` | Nothing readable has been said since — the agent has not started, it stopped before its first token, or (if you landed here from the row below) its closing message was empty. `process.state` from `$TWICC session <ID>` (or `sessions get <ID>` right after a spawn) tells you whether it is still running; the transcript cannot. |
