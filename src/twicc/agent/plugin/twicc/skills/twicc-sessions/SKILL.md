...
The plural of `session <ID> wait-reply`, on sessions **nobody just messaged**: spawned earlier, steered from the UI, messaged by someone else — or ones you messaged with `send-messages` without `--wait-reply` (then pass `--since` an instant taken before the send, or wait on each with `session <id> wait-reply --from <last_line>`). When you send the messages yourself, the simplest form is `send-messages --wait-reply` — it reads each cursor server-side and needs nothing from you. **A send without `--wait-reply` is never followed by a wait without a cursor.**
...
**Each session starts after its own last user message**, so an answer already given is returned (except while a session's compute is not current — e.g. right after a TwiCC restart: then it starts at its current last line, and you pass `--since` an instant before the spawn or the send). `--since` names the cursor as an **ISO 8601 instant** instead (`2026-09-30T14:05:00+00:00`; no offset means UTC; a bare date means its midnight UTC), translated per session. There is no `--from`: line 42 is a different place in every transcript, which is why an instant is what addresses a batch at all. `--since` exists on `sessions wait-reply` and `session <ID> wait-reply` only (the singular also takes `--from`), not on `create-session`, `send-message` or `send-messages`, whose `--wait-reply` reads the cursor server-side.
...
**Resuming a timed-out batch:** re-running resumes, except for a session whose compute is not current; `--since <the instant the batch started>` resumes in every case. The per-session alternative is each block's `since_line_num` handed to `$TWICC session <ID> wait-reply --from`.
...
Stops the agents behind the selected sessions. Only sessions that **currently have a process** are targeted — a stopped one has nothing to stop. **A bare call is refused** (exit `1`): at least one id or one filter, since a bare call would stop every running session. "Stop everything running" stays possible, written on purpose: `--state starting --state assistant_turn --state awaiting_user_input --state user_turn`. Filters select with the same rules as the listing: `$TWICC sessions stop --spawned-by self` stops your children, `--state awaiting_user_input` stops what is blocked on a human. Named ids are added to the filters' selection (unioned), so `$TWICC sessions stop <MANAGER_ID> --descendants <MANAGER_ID>` stops a manager and its subtree. **`sessions stop` never stops the calling session**: named (`self` included) or selected by a filter, it is reported `skipped_self`, counted in `failed`, and nothing is sent for it. `parent`, `--spawn-tree` and `--siblings` still reach beyond your children (`--spawn-tree self` selects you, reported `skipped_self`); `--annotation` alone still selects across every tree — pair it with a filiation scope.
...
Idempotent, and it never fails as a whole: exit 0, `{summary, results}`, one `results` entry per target, keyed by id in selection order, carrying `status` (`stopped` / `rejected` / `failed` / `timeout` / `skipped_*`, `skipped_self` included), `session_known`, `request_uuid`, `provider`, `session_title`, `project_id` and `error`. `summary` holds `total`, `succeeded` (`status: stopped`), `failed` (every other status) and `all_succeeded` (`true` on an empty selection: nothing asked, nothing failed). Exit `1` on a local refusal before anything is stopped: a bare call, `--state dead`, `--timeout` ≤ 0 or conflicting scopes are refused even with no backend; an unknown `--workspace` / `--provider` / `--state` value, a malformed `--annotation` or a `self` / `parent` that cannot be resolved are checked after the backend check, so with no backend they exit `2`; exit `2` when no backend runs. `--force` SIGKILLs immediately instead of letting the turn finalize; `--timeout` (default 30 s) is a wall-clock budget for the whole batch, not per session.
...
For a single session, `$TWICC session <ID> stop` does the same thing — and it is the way to stop your own session (`session self stop`), which `sessions stop` never does.
...
- `--slim` / `--full` — the projection of each row. The reduced one: every field except the payloads you fetch per session (`tasks`, `plan_paths`, `goals`, `layout` — the `has_*` flags `has_tasks`, `has_goals`, `has_plan`, `has_artifacts`, `has_workflows` say what there is to fetch), the redundant timestamps (`mtime`, `last_started_at`, `last_updated_at`, `last_stopped_at`, `last_viewed_at`), the cost breakdown (`self_cost`, `subagents_cost`), `slug`, `browser_url` and `compute_version_up_to_date`; its `process` block is `{state}`. `--full` returns the full payload, the one `session <ID> --full` returns. **Before 2026-10-01 the full payload is the default and `--slim` opts in; from that date the reduced projection is the default and `--slim` is an accepted no-op** — `--full` keeps working on both sides. Until then a call with neither flag prints a one-line notice on stderr (in the RPC envelope's `warnings` key; never on MCP), next to the `--paginated` one. Mutually exclusive (exit `2`). `sessions get` takes them too, placeholders included, so a batch lookup keeps one shape.
...
- `--paginated` — wrap the result in `{items, pagination}` with `limit`, `offset`, `total` and `has_more`, so you know whether another page follows instead of guessing from the page size. Without an explicit `--limit` the page size is **20**. **Before 2026-10-01 the flag is opt-in and a call without it keeps its shape (its rows gain new keys and values now — see the row fields below); from that date the envelope is the only shape and the flag is an accepted no-op.** Passing it works on both sides.
...
- `--paginated` — wrap the result in `{"items": [...]}` — **no `pagination`**: one entry per id asked, nothing to page. **Before 2026-10-01 the result is a bare array and the flag opts in; from that date `{"items": [...]}` is the only shape and the flag is an accepted no-op.** Until then a call without it prints a one-line notice on stderr (in the RPC envelope's `warnings`; never on MCP) — a flagless call prints two, this one then the slim one.
...
- `scratch_dir` — the session's own scratch folder. `orchestration_scratch_dir` — the shared scratch folder of an orchestration tree (the `scratch_dir` annotation), `null` outside one.
- Agent settings (`permission_mode`, `selected_model`, `effort`, `thinking_enabled`, `claude_in_chrome`, `fast_mode`, `context_max`, `question_widget`) — **effective**: the stored value, else the current default (`question_widget` `true` when not chosen); a subagent row keeps its stored values, mostly `null`.
...
- `$TWICC session <session_id|self|parent>` — one session's row, same `--slim` / `--full` (exit 1 when no row has that id, or when `self` / `parent` cannot be resolved). Skill: `twicc-session`.
