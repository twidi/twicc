...
from .notifications import (
    _TASK_NOTIFICATION_TAG,
    _TASK_NOTIFICATION_CLOSE_TAG,
    ParsedTaskNotification,  # noqa: F401 - compatibility re-export
    _extract_task_notification_fields,  # noqa: F401 - compatibility re-export
    _parse_task_notification,
    _is_misrouted_task_result,
    parse_queue_completion,
)
from .subagent_meta import read_subagent_meta, read_subagent_metas, subagents_dir_for_file
...
def _agent_launch_tool_use_id_from_sidecar(session_id: str, task_id: str) -> str | None:
    ...
    if not file_path:
        return None
    meta = read_subagent_meta(subagents_dir_for_file(file_path), task_id)
    return meta.tool_use_id if meta else None
...
class ClaudeCodeSessionCompute(BaseSessionCompute):
    ...
    rebuild_agent_prompt_links = True
    ...
    def extract_agent_spawn_ack(self, parsed_json: dict) -> tuple[str, str] | None:
        ...
        for block in content or []:
            ...
            if isinstance(raw, list):
                raw = "\n".join(part["text"] for part in raw if isinstance(part, dict)
                    and part.get("type") == "text" and isinstance(part.get("text"), str))
            if not isinstance(raw, str) or not isinstance(tool_id, str) or not tool_id:
                continue
            match = re.search(r"Async agent launched successfully.*?agentId:\s*(a[0-9a-f]+)(?=\s|$)", raw, re.DOTALL)
            if match:
                return tool_id, match[1]
        return None
    ...
    def get_subagent_spawn_meta(self, session: Session):
        ...
        if session.type != SessionType.SUBAGENT or not session.parent_session_id:
            return None
        meta = read_subagent_meta(subagents_dir_for_file(session.file_path), session.id)
        if meta is None:
            return None
        return SpawnMetaInfo(meta.parent_agent_id or session.parent_session_id, meta.tool_use_id)

    def get_spawn_metas_for_tree(self, session: Session):
        from ..compute_base import SpawnMetaInfo

        root_id = session.parent_session_id or session.id
        # Workflow launchers live deeper on disk; their normal children still
        # share the root's flat sidecar directory.
        root_file = session.file_path
        if session.parent_session_id:
            root_file = Session.objects.filter(id=root_id).values_list("file_path", flat=True).first() or root_file
        return {agent_id: SpawnMetaInfo(meta.parent_agent_id or root_id, meta.tool_use_id)
                for agent_id, meta in read_subagent_metas(subagents_dir_for_file(root_file)).items()}
